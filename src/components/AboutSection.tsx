...
const AboutSection = () => {
  ...
  const EnhancedInteractiveCanvas = () => {
    ...
    return <div className="relative w-full h-72 bg-gradient-to-br from-slate-900 via-blue-900 to-purple-900 rounded-2xl overflow-hidden">
        {/* Enhanced animated star background */}
        <div className="absolute inset-0">
          {/* Main stars representing alumni */}
          {[...Array(50)].map((_, i) => <div key={`star-${i}`} className="absolute w-1 h-1 bg-white rounded-full alumni-star" style={{
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20">
            ...
            return <line key={`connection-${i}`} x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`} stroke="#ffffff" strokeWidth="0.5" strokeDasharray="2,4" className="alumni-connection" style={{
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
          {connections.map((connection, index) => <line key={index} x1={`${connection.from.position.x}%`} y1={`${connection.from.position.y}%`} x2={`${connection.to.position.x}%`} y2={`${connection.to.position.y}%`} stroke="#FFD700" strokeWidth="2" opacity="0.8" strokeDasharray="4,2" style={{
        ...
        return <div key={value.id} className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-500 ease-in-out z-20" style={{
          left: `${value.position.x}%`,
          top: `${value.position.y}%`,
          transform: `translate(-50%, -50%) scale(${isDiscovered ? 1.1 : isActive ? 1.05 : 0.85})`
        }} onClick={() => handleValueClick(value)}>
              <div className={`relative p-4 rounded-xl bg-gradient-to-r ${value.color} shadow-lg transition-all duration-300 ${isDiscovered ? 'ring-2 ring-yellow-400/50 shadow-yellow-400/25' : ''} ${isActive ? 'ring-2 ring-white/50 shadow-white/25 glow-pulse' : ''}`}>
                <div className="text-white">
                  {value.icon}
                </div>
                
                {/* Discovery indicator */}
                {isDiscovered && <div className="absolute -top-1 -right-1 bg-yellow-400 rounded-full p-1">
                    <Star className="h-3 w-3 text-yellow-800 fill-current" />
                  </div>}

                {/* Active indicator */}
                {isActive && <div className="absolute -top-2 -right-2">
        ...
        {expandedValue && <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
        ...
        {/* Particle effects */}
        <ParticleSystem active={showParticles.active} centerX={showParticles.x} centerY={showParticles.y} color="#FFD700" />

        {/* Game guidance */}
        {!gameComplete && currentValue && <div className="absolute bottom-3 left-3 bg-black/70 text-white px-3 py-1 rounded-full text-sm flex items-center gap-2 z-30">
        ...
        <div className="absolute bottom-3 right-3 bg-white/80 rounded-full px-3 py-1 z-30">
          <div className="flex items-center gap-2 text-gray-800">
            <div className="w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-yellow-400 to-orange-500 transition-all duration-1000 rounded-full" style={{
              width: `${progress}%`
            }} />
            </div>
        ...
        {/* Completion overlay */}
        {gameComplete && <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-40">
            <div className="text-center text-white">
              ...
              <button onClick={resetGame} className="mt-4 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-2 rounded-full hover:scale-105 transition-transform duration-300 flex items-center gap-2 mx-auto">
  ...
  return <section className="py-20 bg-gradient-to-br from-slate-50 via-white to-cpscs-light/50 overflow-hidden">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="text-center mb-12">
          <h3 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-cpscs-blue via-purple-600 to-cpscs-blue bg-clip-text text-transparent mb-4 leading-tight">Our Values</h3>
          
          <p className="text-gray-700 text-xl font-light max-w-2xl mx-auto">Learn about our values through this fun little game.</p>
        </div>
